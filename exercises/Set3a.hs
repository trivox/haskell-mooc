...
capitalize xs = unwords [ capitalizeFirst wrd | wrd <- words xs ]
...
powerHelper cp k max = if kp > max  then [] else kp : powerHelper (cp+1) k max
    where kp = k^cp 

powers :: Int -> Int -> [Int]
powers k max 
    | k > max   = [1]
    | otherwise = 1 : k : powerHelper 2 k max
...
whileRight check x = process (check x)
    where process (Left v)  = v
          process (Right v) = process (check v) 
...
joinToLength _ [] = []
joinToLength l xs = [ x ++ y | x <- xs, y <- xs, (length x) + (length y) == l ]
...
multiApp f    []  _   = f []
multiApp ffin arf arg = ffin [ f arg | f <- arf ] 
...
interpreter commands = helpInterpret 0 0 commands
    where
        helpInterpret _ _ [] = []
        helpInterpret x y ("up":cs)     = helpInterpret x     (y+1) cs 
        helpInterpret x y ("down":cs)   = helpInterpret x     (y-1) cs 
        helpInterpret x y ("right":cs)  = helpInterpret (x+1) y     cs 
        helpInterpret x y ("left":cs)   = helpInterpret (x-1) y     cs 
        helpInterpret x y ("printX":cs) = show x : (helpInterpret x y cs) 
        helpInterpret x y ("printY":cs) = show y : (helpInterpret x y cs) 
        helpInterpret x y (_:cs)   = helpInterpret x y cs 
