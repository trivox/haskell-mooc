...
totalPrice (MkShoppingEntry _ p n) = p * (fromIntegral n)
...
buyOneMore (MkShoppingEntry s p n) = MkShoppingEntry s p (n+1)
...
fromList []         = Empty
fromList ((a,b):xs) = Pair a b (fromList xs)
...
toBin n
    ...
    where
        helper n 
            | n == 0    = End
            | odd n     = I (helper ((n-1) `div` 2))
            | otherwise = O (helper (n `div` 2))
