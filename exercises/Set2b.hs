...
binomial n 0 = 1
binomial 0 k = 0
binomial n k = if (n > 0) && (k > 0) 
               then (binomial (n-1) k) + (binomial (n-1) (k-1))
               else 0
...
leftpad str l = 
    (
        if pad > 0
        then (take pad (repeat ' '))
        else ""
    ) ++ str
    where pad = l - (length str)
...
countdown i = "Ready! " ++ 
              (intercalate " " (map myShow [i,(i-1)..1])) ++ 
              " Liftoff!"
...
findDivisor n k 
    | k < n     = if (n `mod` k) > 0 then findDivisor n (k+1) else k
    | k == n    = k
    | otherwise = findDivisor k n
